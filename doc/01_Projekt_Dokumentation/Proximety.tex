
%------------------------------------------------
% Semesterarbeit ESA
% 
% Autor:	Sandro Dallo sandro.dallo@students.ffhs.ch, Patrick Bösch patrick.boesch@students.ffhs.ch
%			Jonas Alder jonas.alder@students.ffhs.ch, Andy Villiger andy.villiger@students.ffhs.ch
% Datum:	17.09.2014
%
% Version:	1.0
%
%------------------------------------------------

%-- Preamble ------------------------------------
\documentclass[10pt,a4paper,twoside]{article}

%-- Nur für Debugging
%\usepackage{showframe}
\usepackage{float}
\usepackage{blindtext}
\usepackage{url} % Bibtext url
\usepackage[space]{grffile}
\usepackage{multirow}

%-- Import packages -----------------------------
\input{allgemein/packages.tex}
%-- Import Dokumenten Infos ---------------------
\input{documentinfo.tex}
%-- Import Seitenstil ---------------------------
\input{allgemein/pagestyle.tex}
%-- Import Definitions --------------------------
\input{allgemein/definitionen.tex}
%-- Import Kopf und Fusszeilen ------------------
\input{allgemein/kopffusszeilen.tex}
%-- Import Kommandos und Environments -----------
\input{allgemein/commands.tex}
%-- Import Tabellendefinitionen -----------
\input{allgemein/tabellen.tex}
%-- Tabllenabstände
%\setlength{\tabcolsep}{10mm} % Column
\renewcommand{\arraystretch}{1.5} % Row
% -- Code Styling -------------------------------
\definecolor{lightgray}{RGB}{224,224,224}
\begin{document}
%-- Deckblatt -----------------------------------
\input{allgemein/deckblatt.tex}
% Dokumentenverlauf
\input{history.tex}
%-- Inhaltsverzeichnis --------------------------
\setcounter{tocdepth}{5} % Tiefe des Inhaltsverzeichnis
\setcounter{secnumdepth}{5} % Nummerierungstiefe
\tableofcontents
\newpage 
%-- Allgemeines --------------------------
\section{Allgemeines}
\input{allgemeines.tex}
%-- Anforderungen --------------------------
\section{Anforderungen}
\input{anforderungen.tex}
\section{GUI Prototypes}
\subsection{Einleitung}
\input{gui_einleitung.tex}
\subsection{Variante 1}
\input{screens_variante_1.tex}
\newpage
\subsection{Variante 2}
\input{screens_variante_2.tex}
\newpage
\subsection{Vergleich und Entscheid}
\input{vergleich.tex}
\newpage
\section{Architektur}
\input{architektur.tex}
\newpage
\section{Testing}
\input{testing.tex}
%--- Appendix -------------------
\newpage
\appendix
\listoffigures
\listoftables

%-- Import Literature ----
\newpage
\renewcommand{\refname}{Quellenverzeichnis} 
\bibliography{allgemein/literatur}
\bibliographystyle{alpha}
\end{document}
